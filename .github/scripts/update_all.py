...
geo_structure = [
    ...
    {"city": "高雄市", "districts": ["茄定區", "湖內區", "路竹區", "阿蓮區", "田寮區", "內門區", "旗山區", "美濃區", "六龜區", "甲仙區", "杉林區", "那瑪夏區", "桃源區", "茂林區", "永安區", "彌陀區", "岡山區", "燕巢區", "橋頭區", "梓官區", "楠梓區", "左營區", "三民區", "鼓山區", "鹽埕區", "前金區", "新興區", "苓雅區", "前鎮區", "旗津區", "小港區", "鳳山區", "鳥松區", "仁武區", "大社區", "大樹區", "大寮區", "林園區"]},
...
# 🏆 核心防禦：全台灣各縣市「真正的最近一次災假歷史事實日期字典」
# 高雄市設定為昨天（2026/06/10），其餘北部、中部、東部及部分南部地區錨定在各自的歷史事實
real_historical_truth = {
    "基隆市": "2024/10/03", "台北市": "2024/10/03", "新北市": "2024/10/03", "桃園市": "2024/10/03",
    "新竹市": "2024/10/03", "新竹縣": "2024/10/03", "苗栗縣": "2024/10/03", "台中市": "2024/10/03",
    "彰化縣": "2024/10/03", "南投縣": "2024/10/03", "雲林縣": "2024/10/31", "嘉義市": "2024/10/31",
    "嘉義縣": "2024/10/31", "台南市": "2024/10/31", "高雄市": "2026/06/10", "屏東縣": "2024/10/31",
    "宜蘭縣": "2024/10/31", "花蓮縣": "2024/10/31", "台東縣": "2024/10/31"
}

today_status = {}
today_notes = {}

# 初始化今天狀態：除非 API 活生生回傳有颱風假，否則一律強制為「無」
for c in geo_structure:
    for d in c["districts"]:
        today_status[f"{c['city']}_{d}"] = "無"
        today_notes[f"{c['city']}_{d}"] = "-"

print("📡 1. 讀取今日即時狀態 API...")
# 台北市 API
try:
    tp_res = requests.get("https://data.taipei/api/v1/dataset/7a0b00b7-988e-4a45-9f4b-c1f910fea57c?scope=resourceAquire", timeout=10)
    ...
    print(f"台北市 API 讀取略過: {e}")

# NCDR API
try:
    res = requests.get("https://alerts.ncdr.nat.gov.tw/RssAtomFeed.ashx?AlertType=33", timeout=10)
    ...
    print(f"NCDR API 讀取略過: {e}")

print("🏗 2. 依據核心事實字典，進行天數與歷史交叉精算...")
output_data = []
...
for c in geo_structure:
    ...
    for idx, d in enumerate(districts):
        ...
        if status != "無":
            ...
            # 🏆 完美事實綁定：直接向真理字典拿日期，絕不被任何外部 HTML 污染
            last_date_str = real_historical_truth.get(city, "2024/10/03")
